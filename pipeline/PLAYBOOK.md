...
## 6. Hero images — current status

**There is no image-generation connector installed in this org.** The old Routine called a
`gemini-image-gen` connector; `ListConnectors` confirms no such connector exists (checked
2026-09-10). Every hero-image step in the previous build was therefore failing or silently
skipped.

Until one is added, stage 05 is **agent brief + human pick**:

1. The draft run writes `drafts/<slug>/image-brief.md` — a ready-to-paste generation prompt,
   composition notes, and alt text. See `pipeline/IMAGE-BRIEF.md`.
2. A human generates or selects the image and drops it in as `drafts/<slug>/hero.png`
   (16:9, ≥1600px wide).
3. The packaging step embeds whatever `hero.png` is present. If none is present, it proceeds
   with a placeholder and says so — a missing image must not block review of the text.

**If an image connector is later installed:** add its exact tool name here, and the draft
Routine will use it automatically — each run checks this section for a named tool before
falling back to the brief.

Named image tool: _(none — fallback to brief)_
