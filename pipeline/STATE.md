...
### Upload-test artifacts removed

Three upload-test files left by an earlier Routine run — `CH-Upload-Test.docx` (16,359 B),
`ZZ-Upload-Test-15KB.docx` (22,575 B) and `ZZ-Upload-Test-FullDraft.docx` (17,159 B) — deleted
on Matt's instruction. All three carried the healthy stored-size signature, so that run had
independently found large uploads working; `CH-Upload-Test.docx` said so in its own body
("Generated by the blog content pipeline as an upload diagnostic. Safe to delete."), and
`ZZ-Upload-Test-15KB.docx` was a copy of the Equipment Room draft, which survives under its
canonical name.

Each ID was confirmed before deleting, per the new §5 rule: `ZZ-Upload-Test-FullDraft` from
`sharepoint_search`'s structured `name`/`id` fields, which also cross-checked the listing parse,
and the other two by reading their content back first. Folder re-listed afterwards: both blog
drafts present at 22,473 B and 22,492 B, and nothing else removed.
